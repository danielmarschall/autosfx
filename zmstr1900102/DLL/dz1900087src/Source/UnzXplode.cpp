...
//#endif
  ...
  * bd = 8 worked marginally better for large compressed sizes. */
  bl = 7;
//  bd = (fcsize + fincnt) > 200000L ? 8 : 7;
  bd = (unsigned long)(fcsize + fincnt) > 200000L ? 8 : 7;

  /* With literal tree--minimum match length is 3 */
  fhufts = 0;
  /* initialize huft's malloc'ed */
  if (flrec.general_purpose_bit_flag & 4)
  {
    bb = 9;
    ...
    if ((r = get_tree(l, 64)) != 0)
      return (int)r;
    if (flrec.general_purpose_bit_flag & 2)
    {
      /* true if 8K */

...
int UnzOpr::open_outfile(void)
{
	...
	{
		DWORD dwAttrs = GetFileAttributes(ffilename);
		if (dwAttrs != INVALID_FILE_ATTRIBUTES)
		{
			...
			if ((dwAttrs & FILE_ATTRIBUTE_READONLY))
			{
				if (Verbose)
					...
					ffilename.c_str());
				if (!SetFileAttributes(ffilename,
						dwAttrs&~FILE_ATTRIBUTE_READONLY) && Verbose < 0)
					Notify(IVERBOSE, _T("could not reset read-only %s [%s]"),
					ffilename.c_str(), SysMsg().c_str());
			}
			if (!DeleteFile(ffilename))
			{
				if (Verbose)
...
int UnzSup::part_flush(uch * rawbuf, ulg size, int unshrink)
{
	 ...
	 *--------------------------------------------------------------------------- */
	fcrc32val = crc32(fcrc32val, rawbuf, (::extent)size);
...
//	{
	...
	if (q > transbuf)
	{
		unsigned long k, sz = (unsigned)(q - transbuf);
		if ((!fUnzOutfile->Write(transbuf, sz, &k) || k != sz))
			return disk_error();
	}
//	}
	return 0;
...
time_t dos_to_unix_time(unsigned ddate, unsigned dtime)
{
	...
	m_time = (time_t)((long)(days + dy) * 86400L + (long)hh * 3600L + (long)
		(mm * 60 + ss));
...
int UnzOpr::do_string(unsigned int len, int option)
{
	...
	case DS_FN:
		...
		{
			file_name.Empty();
			if (readbuf(file_name.GetBuffer(len), len) == 0)
				return PK_EOF;
			file_name.ReleaseBuffer(len);
		}
...
int UnzSup::zlseek(ZInt64 abs_offset)
{
	...
	{
		fincnt += (finptr - finbuf) - (int)inbuf_offset;
		finptr = finbuf + (int)inbuf_offset;
	}
